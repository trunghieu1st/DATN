...
async function loadOption(productId) {
    ...
    const response = await fetch('http://localhost:8080/api/v1/product/option' + link, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': 'Bearer ' + accessToken
    ...
    editButtons.forEach(editButton => {
        editButton.addEventListener('click', async function () {
            ...
            const response3 = await fetch('http://localhost:8080/api/v1/product/option/' + producOptionId, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer ' + accessToken
...
async function deleteProductOption(productOptionId) {
    const response = await fetch(`http://localhost:8080/api/v1/product/option/${productOptionId}`, {
        method: 'DELETE',
        headers: {
            'Authorization': 'Bearer ' + accessToken
        },
